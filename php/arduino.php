...
$maquinas = [];
$result = $conexao->query("SELECT DISTINCT fk_id_maquina FROM dados_maquinas");
while ($row = $result->fetch_assoc()) {
    $maquinas[] = $row['fk_id_maquina'];
}
...
foreach ($maquinas as $fk_id_maquina) {
    ...
    ");
    $stmtUpdate->bind_param("iiiis", $temperatura, $consumo, $umidade, $hora, $fk_id_maquina);
    $stmtUpdate->execute();
